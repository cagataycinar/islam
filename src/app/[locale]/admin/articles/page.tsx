...
async function fetchArticles(searchParams: { q?: string; page?: string }) {
  try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="./articles/new" className="bg-green-600 text-white px-4 py-2 rounded-md">
          New Article
        </Link>
      </div>
            ...
            {items.map((a: any) => (
              <tr key={a._id}>
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`./articles/${a._id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                </td>
              </tr>
            ))}
            {items.length === 0 && (
              <tr>
                <td className="px-4 py-6 text-gray-500" colSpan={3}>No articles found.</td>
              </tr>
            )}
          </tbody>
        </table>
