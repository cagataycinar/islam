...
async function fetchHadiths(searchParams: { q?: string; page?: string }) {
  try {
    ...
    const res = await fetch(`/api/admin/hadiths?${query.toString()}`, { cache: 'no-store' });
...
export default async function AdminHadithPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  const params = await searchParams;
  const { items, total, page, pageSize } = await fetchHadiths(params || {});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="./hadith/new" className="bg-green-600 text-white px-4 py-2 rounded-md">New Hadith</Link>
      </div>
      ...
      <div className="bg-white rounded-md border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Text (EN)</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Text (TR)</th>
            ...
            {items.map((a: HadithListItem) => (
              <tr key={a._id}>
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`./hadith/${a._id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                </td>
              </tr>
            ))}
            {items.length === 0 && (
              <tr>
                <td className="px-4 py-6 text-gray-500" colSpan={3}>No hadiths found.</td>
              </tr>
            )}
          </tbody>
        </table>
