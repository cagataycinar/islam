...
async function fetchBlogs(searchParams: { q?: string; page?: string }) {
  try {
    const query = new URLSearchParams();
    if (searchParams.q) query.set('q', searchParams.q);
    if (searchParams.page) query.set('page', searchParams.page);
    const res = await fetch(`/api/admin/blogs?${query.toString()}`, { cache: 'no-store' });
...
export default async function AdminBlogsPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  const params = await searchParams;
  const { items, total, page, pageSize } = await fetchBlogs(params || {});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="./blogs/new" className="bg-green-600 text-white px-4 py-2 rounded-md">New Blog</Link>
      </div>
      ...
      <div className="bg-white rounded-md border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map((a: BlogListItem) => (
              <tr key={a._id}>
                <td className="px-4 py-3">{a.title?.en}</td>
                <td className="px-4 py-3">{a.title?.tr}</td>
                <td className="px-4 py-3 text-right">
                  <Link href={`./blogs/${a._id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                </td>
              </tr>
            ))}
            {items.length === 0 && (
              <tr>
                <td className="px-4 py-6 text-gray-500" colSpan={3}>No blogs found.</td>
              </tr>
            )}
          </tbody>
        </table>
